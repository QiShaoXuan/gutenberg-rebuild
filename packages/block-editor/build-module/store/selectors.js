 ...
 */
import { castArray, flatMap, first, get, includes, isArray, isBoolean, last, map, orderBy, reduce, some } from 'lodash';
import createSelector from 'rememo';
/**
 * WordPress dependencies
 */

import { getBlockType, getBlockTypes, hasBlockSupport, hasChildBlocksWithInserterSupport } from '@wordpress/blocks';
/***
 * Module constants
 */
...
export var getBlockAttributes = createSelector(function (state, clientId) {
  var block = state.blocks.byClientId[clientId];
...
export var getBlock = createSelector(function (state, clientId) {
  ...
  return [].concat(_toConsumableArray(getBlockAttributes.getDependants(state, clientId)), [getBlockDependantsCacheBust(state, clientId)]);
});
export var __unstableGetBlockWithoutInnerBlocks = createSelector(function (state, clientId) {
  ...
  return [state.blocks.byClientId[clientId]].concat(_toConsumableArray(getBlockAttributes.getDependants(state, clientId)));
});
/**
...
export var getMultiSelectedBlocks = createSelector(function (state) {
  ...
  return [].concat(_toConsumableArray(getSelectedBlockClientIds.getDependants(state)), [state.blocks.byClientId, state.blocks.order, state.blocks.attributes, getPostMeta(state)]);
});
/**
...
var isAncestorOf = createSelector(function (state, possibleAncestorId, possibleDescendentId) {
...
var canIncludeReusableBlockInInserter = function canIncludeReusableBlockInInserter(state, reusableBlock, rootClientId) {
  ...
  var referencedBlockType = getBlockType(referencedBlockName);
...
export var hasInserterItems = createSelector(function (state) {
  ...
  var hasReusableBlock = some(getReusableBlocks(state), function (block) {
    return canIncludeReusableBlockInInserter(state, block, rootClientId);
  });
